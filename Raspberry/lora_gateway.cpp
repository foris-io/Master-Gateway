...
///////////////////////////////
         ...
         for ( ; a<tmp_length; a++,b++) {
           ...
           //printf("\ntest in forloop\n");
           //strcat(str,sx1272.packet_received.data[a]);
           //str[ii]=(char)sx1272.packet_received.data[a];
	   //ii++;
...
//	printf("4th char: %c",cmd[i]);
	...
	salinity[j]='\0';
	printf("\ntemp:%s\nhum:%s\nmoist:%s\nsal:%s\n",temperature,humidity,moisture,salinity);
//---creating JSON object----

         char jsonObj[512];
         // printf("--no exit--");

          strcpy(jsonObj,"{\"TimeStamp\":");
	  ...
	  printf("time: %s",timeStamp);
         ...
         // strftime(TimeStamp, sizeof TimeStamp, "%F %T %Z", gmtime(&t));
          ...
          strcat(jsonObj,timeStamp);
          strcat(jsonObj,"\"");
          // strcat(jsonObj,(uint8_t*)tmst);
	 strcat(jsonObj,",");
         strcat(jsonObj,"\"masterId\":");
         strcat(jsonObj,"\"");
         strcat(jsonObj,"m001");
         strcat(jsonObj,"\"");
	 strcat(jsonObj,",");
 
         strcat(jsonObj,"\"nodeId\":");
         strcat(jsonObj,"\"");
         strcat(jsonObj,"n001");
         strcat(jsonObj,"\"");
	 strcat(jsonObj,",");
	 ...
	 strcat(jsonObj,",");
	  strcat(jsonObj,"\"");
         ...
         strcat(jsonObj,",");
          strcat(jsonObj,"\"");
         strcat(jsonObj,"moisture\":");
         strcat(jsonObj,moisture);
         strcat(jsonObj,",");
	 strcat(jsonObj,"\"");
         strcat(jsonObj,"salinity\":");
         strcat(jsonObj,salinity);
         //strcat(jsonObj,",");
	...
	if(curl) {
		//curl_easy_setopt(curl, CURLOPT_URL, "https://foris:grover2014@foris.cloudant.com/sensor-data");
		curl_easy_setopt(curl, CURLOPT_URL, "https://474306cf-8215-4090-879c-4fb2f25624a6-bluemix:a0807af23864f72d10fc698e9d5e2a2a739411860d67625aabf73061d22acccd@474306cf-8215-4090-879c-4fb2f25624a6-bluemix.cloudant.com/sensor-data");
